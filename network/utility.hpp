...
public:
    ...
    static CaseInsensitiveMultimap parse(
        ...
        for (std::size_t c = 0; c < query_string.size(); ++c) {
            if (query_string[c] == '&') {
                auto name = query_string.substr(
                  name_pos,
                  (name_end_pos == std::string::npos ? c : name_end_pos) -
                    name_pos);
                if (!name.empty()) {
                    auto value =
...
public:
    ...
    public:
        ...
        {
        public:
            /// Parse Set-Cookie or Content-Disposition from given header field
            /// value. Attribute values are percent-decoded.
                ...
                for (std::size_t c = 0; c < value.size(); ++c) {
                    if (name_start_pos == std::string::npos) {
                        ...
                        if (name_end_pos == std::string::npos) {
                            if (value[c] == ';') {
                                result.emplace(value.substr(name_start_pos,
                                                            c - name_start_pos),
                                               std::string());
                            ...
                            if (value_start_pos == std::string::npos) {
                                ...
                                result.emplace(
                                  value.substr(name_start_pos,
                                               name_end_pos - name_start_pos),
                                  Percent::decode(value.substr(
                                    value_start_pos, c - value_start_pos)));
                                name_start_pos = std::string::npos;
                                name_end_pos = std::string::npos;
        ...
        if (getline(stream, line) &&
            ...
            if (path_and_query_string_end != std::string::npos) {
                if (query_start != std::string::npos) {
                    path =
                      line.substr(method_end + 1, query_start - method_end - 2);
                    query_string = line.substr(
                      query_start, path_and_query_string_end - query_start);
                } else
                    path =
                ...
                if ((protocol_end =
                       line.find('/', path_and_query_string_end + 1)) !=
                    std::string::npos) {
                    if (line.compare(path_and_query_string_end + 1,
                                     protocol_end - path_and_query_string_end -
                                       1,
                                     "HTTP") != 0)
                        return false;
                    version = line.substr(protocol_end + 1,
                                          line.size() - protocol_end - 2);
                } else
                    return false;
...
{
public:
    /// Returns the given std::chrono::system_clock::time_point as a string with
    /// the following format: Wed, 31 Jul 2019 11:34:23 GMT.
      ...
      const std::chrono::system_clock::time_point time_point) noexcept
    {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(time_point -
                                                             last_time_point)
                .count() == 0 &&
            !result_cache.empty())
            return result_cache;
...
public:
    ...
    public:
        ~SharedLock() noexcept { count.fetch_sub(1); }
    };
    ...
    /// The shared lock ensures that a potential destructor call is delayed
