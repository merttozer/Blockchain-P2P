...
public:
    /// Response class where the content of the response is sent to client when
      ...
      , public std::ostream
    {
        ...
        std::list<std::pair<std::shared_ptr<asio::streambuf>,
                            std::function<void(const error_code&)>>>
          send_queue GUARDED_BY(send_queue_mutex);

        Response(std::shared_ptr<Session> session_,
                 long timeout_content) noexcept
          : std::ostream(nullptr)
        ...
        void write_header(const CaseInsensitiveMultimap& header, size_type size)
        {
            ...
            for (auto& field : header) {
                if (!content_length_written &&
                    ...
                    content_length_written = true;
                         ...
                         case_insensitive_equal(field.first,
                                                "transfer-encoding") &&
                         case_insensitive_equal(field.second, "chunked"))
                    chunked_transfer_encoding = true;
                else if (!event_stream &&
                         case_insensitive_equal(field.first, "content-type") &&
                         case_insensitive_equal(field.second,
                                                "text/event-stream"))
                    event_stream = true;
            ...
            if (!content_length_written && !chunked_transfer_encoding &&
                !event_stream && !close_connection_after_response)
                ...
                asio::async_write(
                  *self->session->connection->socket,
                  ...
                  [self](const error_code& ec,
                      ...
                      auto lock = self->session->connection->handler_runner
                                    ->continue_lock();
                      if (!lock)
                          return;
                      {
                          ...
                          if (!ec) {
                              ...
                              std::list<std::function<void(const error_code&)>>
                                callbacks;
        ...
        void send_on_delete(const std::function<void(const error_code&)>&
                              callback = nullptr) noexcept
        {
            auto buffer = streambuf->data();
            auto self =
              this->shared_from_this(); // Keep Response instance alive through
                                        // the following async_write
            post(session->connection->write_strand, [self, buffer, callback] {
                ...
                asio::async_write(
                  *self->session->connection->socket,
                  buffer,
                  [self, callback](const error_code& ec,
                                   std::size_t /*bytes_transferred*/) {
                      auto lock = self->session->connection->handler_runner
                                    ->continue_lock();
                      if (!lock)
    ...
    public:
        ...
        void send(
          std::function<void(const error_code&)> callback = nullptr) noexcept
        {
            std::shared_ptr<asio::streambuf> streambuf =
              std::move(this->streambuf);
            this->streambuf =
              std::unique_ptr<asio::streambuf>(new asio::streambuf());
        ...
        /// Convenience function for writing status line, potential header
        /// fields, and empty content.
        void write(
        ...
        /// Convenience function for writing success status line, header fields,
        /// and content.
        void write(
          std::string_view content,
          const CaseInsensitiveMultimap& header = CaseInsensitiveMultimap())
        ...
        /// Convenience function for writing success status line, and header
        /// fields.
        ...
        /// If set to true, force server to close the connection after the
    ...
    public:
        ...
        {
            return std::string(asio::buffers_begin(streambuf.data()),
    ...
    class Request
    {
        ...
        std::string optimization =
          std::to_string(0); // TODO: figure out what goes wrong in gcc
                             // optimization without this line

        Request(std::size_t max_request_streambuf_size,
                const std::shared_ptr<Connection>& connection_) noexcept
          : streambuf(new asio::streambuf(max_request_streambuf_size))
          , content_streambuf(max_request_streambuf_size)
          , connection(connection_)
          , content(content_streambuf)
        {
        }
        Request(std::size_t max_request_streambuf_size,
                const std::shared_ptr<Connection>& connection_,
                std::unique_ptr<asio::streambuf>&& previous_streambuf) noexcept
          : streambuf(std::move(previous_streambuf))
          , content_streambuf(max_request_streambuf_size)
          , connection(connection_)
          , content(content_streambuf)
        {
    ...
    public:
        ...
        /// The result of the resource regular expression match of the request
        /// path.
        ...
        /// Deprecated, please use remote_endpoint().address().to_string()
        /// instead.
        SW_DEPRECATED std::string remote_endpoint_address() const noexcept
        {
            try {
                if (auto connection = this->connection.lock())
                    return connection->socket->lowest_layer()
                      .remote_endpoint()
                      .address()
                      .to_string();
            } catch (...) {
...
protected:
    ...
    public:
        ...
        Connection(std::shared_ptr<ScopeRunner> handler_runner_,
                   Args&&... args) noexcept
          : handler_runner(std::move(handler_runner_))
          , socket(new socket_type(std::forward<Args>(args)...))
          , write_strand(get_executor(socket->lowest_layer()))
        {
        }
        ...
        std::unique_ptr<socket_type>
          socket; // Socket must be unique_ptr since
                  // asio::ssl::stream<asio::ip::tcp::socket> is not movable

        /**
         * Needed for TLS communication where async_write could be called
         * outside of the io_context runners. For more information see
         * https://stackoverflow.com/a/12801042.
         */
        strand write_strand;
        ...
        void set_timeout(long seconds) noexcept
        {
            ...
            std::weak_ptr<Connection> self_weak(
              this->shared_from_this()); // To avoid keeping Connection instance
    ...
    public:
        Session(std::size_t max_request_streambuf_size,
                std::shared_ptr<Connection> connection_) noexcept
        ...
        Session(std::size_t max_request_streambuf_size,
                ...
                std::unique_ptr<asio::streambuf>&& previous_streambuf) noexcept
          : connection(std::move(connection_))
          , request(new Request(max_request_streambuf_size,
                                connection,
                                std::move(previous_streambuf)))
        {
        }
...
public:
    ...
    public:
        /// Port number to use. Defaults to 80 for HTTP and 443 for HTTPS. Set
        /// to 0 get an assigned port.
        unsigned short port;
        ...
        /// Timeout on request/response content completion. Defaults to 300
        /// seconds.
        long timeout_content = 300;
        /// Maximum size of request stream buffer. Defaults to architecture
        /// maximum. Reaching this limit will result in a message_size error
        /// code.
        ...
        std::string address;
        /// Set to false to avoid binding the socket to an address that is
        /// already in use. Defaults to true.
...
private:
    ...
    public:
        ...
        regex_orderable(std::string regex_str_)
          : regex::regex(regex_str_)
          , str(std::move(regex_str_))
        {
        }
...
public:
    ...
    /// The callback argument is called after the server is accepting
    /// connections, where its parameter contains the assigned port.
    void start(
      const std::function<void(unsigned short /*port*/)>& callback = nullptr)
    {
        ...
        if (!config.address.empty())
            endpoint = asio::ip::tcp::endpoint(make_address(config.address),
                                               config.port);
        else
            endpoint =
              asio::ip::tcp::endpoint(asio::ip::tcp::v6(), config.port);
        ...
        if (!acceptor)
            acceptor = std::unique_ptr<asio::ip::tcp::acceptor>(
        ...
        try {
            ...
            if (error.code() == asio::error::address_family_not_supported &&
                ...
                endpoint =
                  asio::ip::tcp::endpoint(asio::ip::tcp::v4(), config.port);
                acceptor->open(endpoint.protocol());
            } else
                throw;
        }
        acceptor->set_option(
          asio::socket_base::reuse_address(config.reuse_address));
        if (config.fast_open) {
#if defined(__linux__) && defined(TCP_FASTOPEN)
            const int qlen =
              5; // This seems to be the value that is used in other examples.
            error_code ec;
            acceptor->set_option(
        ...
        if (callback)
            post(*io_service, [callback, port] { callback(port); });

        if (internal_io_service) {
            // If thread_pool_size>1, start m_io_service.run() in
            // (thread_pool_size-1) threads for thread-pooling
            threads.clear();
...
protected:
        ...
        asio::async_read_until(
          ...
          *session->request->streambuf,
          ...
          [this, session](const error_code& ec, std::size_t bytes_transferred) {
              ...
              if (!lock)
                  return;
              session->request->header_read_time =
              ...
              if (!ec) {
                  ...
                  // async_read_until operation, the streambuf may contain
                  // additional data beyond the delimiter" The chosen solution
                  // is to extract lines from the stream directly when parsing
                  // the header. What is left of the streambuf (maybe some bytes
                  // of the content) is appended to in the async_read-function
                  // below (for retrieving content).
                  ...
                  if (!RequestMessage::parse(istream,
                  ...
                  auto header_it =
                    session->request->header.find("Content-Length");
                  if (header_it != session->request->header.end()) {
                      ...
                      if (content_length >
                          ...
                          auto response =
                            std::shared_ptr<Response>(new Response(
                      ...
                      if (num_additional_bytes > 0) {
                          auto content_bytes = std::min<std::size_t>(
                            content_length,
                            (unsigned long long)num_additional_bytes);
                          auto& source = *session->request->streambuf;
                          auto& target = session->request->content_streambuf;
                          target.commit(
                            asio::buffer_copy(target.prepare(content_bytes),
                                              source.data(),
                                              content_bytes));
                      ...
                      if (content_length > num_additional_bytes) {
                          asio::async_read(
                            *session->connection->socket,
                            session->request->content_streambuf,
                  ...
                  } else if ((header_it = session->request->header.find(
                                "Transfer-Encoding")) !=
                               session->request->header.end() &&
                             header_it->second == "chunked") {
                      // Expect hex number to not exceed 16 bytes (64-bit
                      // number), but take into account previous additional read
                      // bytes
                      auto chunk_size_streambuf =
                        std::make_shared<asio::streambuf>(std::max<std::size_t>(
                          16 + 2, session->request->streambuf->size()));

                      // Move leftover bytes
                      auto& source = *session->request->streambuf;
                      auto& target = *chunk_size_streambuf;
                      target.commit(asio::buffer_copy(
      ...
      const std::shared_ptr<asio::streambuf>& chunk_size_streambuf)
    {
        asio::async_read_until(
          ...
          *chunk_size_streambuf,
          "\r\n",
          [this, session, chunk_size_streambuf](const error_code& ec,
              ...
              if (!ec) {
                  ...
                  if (chunk_size + session->request->content_streambuf.size() >
                      session->request->content_streambuf.max_size()) {
                  ...
                  auto read_next =
                    [this, chunk_size](const std::shared_ptr<Session>& session,
                                       const std::shared_ptr<asio::streambuf>&
                                         chunk_size_streambuf) {
                        if (chunk_size == 0) {
                            if (chunk_size_streambuf->size() > 0) {
                                // Remaining bytes are the beginning of the next
                                // request
                                auto& source = *chunk_size_streambuf;
                                auto& target = *session->request->streambuf;
                                target.commit(asio::buffer_copy(
                                  target.prepare(chunk_size_streambuf->size()),
                                  source.data(),
                                  chunk_size_streambuf->size()));
                                source.consume(chunk_size_streambuf->size());
                            }
                            this->find_resource(session);
                        } else
                            this->read_chunked_transfer_encoded(
                  ...
                  auto bytes_to_move =
                    std::min<std::size_t>(chunk_size, num_additional_bytes);
                  if (bytes_to_move > 0) {
                      // Move leftover bytes
                      auto& source = *chunk_size_streambuf;
                      auto& target = session->request->content_streambuf;
                      target.commit(
                        asio::buffer_copy(target.prepare(bytes_to_move),
                                          source.data(),
                                          bytes_to_move));
                  ...
                  if (chunk_size > num_additional_bytes) {
                      asio::async_read(
                        *session->connection->socket,
                        session->request->content_streambuf,
                        asio::transfer_exactly(chunk_size -
                                               num_additional_bytes),
                        [this, session, chunk_size_streambuf, read_next](
                          const error_code& ec, size_t /*bytes_transferred*/) {
                            ...
                            if (!ec) {
                                // Remove "\r\n"
                                auto null_buffer =
                                  std::make_shared<asio::streambuf>(2);
                                asio::async_read(
                                  *session->connection->socket,
                                  *null_buffer,
                             ...
                             num_additional_bytes) { // If only end of chunk
                                                     // remains unread (\n or
                                                     // \r\n)
                      // Remove "\r\n"
                      ...
                      asio::async_read(
                        *session->connection->socket,
                        *null_buffer,
                        asio::transfer_exactly(2 + chunk_size -
                                               num_additional_bytes),
                        [this,
                         session,
                         chunk_size_streambuf,
    ...
    void find_resource(const std::shared_ptr<Session>& session)
    {
        ...
        for (auto& regex_method : resource) {
            ...
            if (it != regex_method.second.end()) {
                regex::smatch sm_res;
                if (regex::regex_match(
                      session->request->path, sm_res, regex_method.first)) {
                 ...
                 resource_function)
    {
                  ...
                  if (!ec) {
                      ...
                      if (response->session->request->http_version >= "1.1") {
                          auto new_session = std::make_shared<Session>(
                            this->config.max_request_streambuf_size,
                            response->session->connection,
                            std::move(response->session->request->streambuf));
                          this->read(new_session);
                          return;
                      }
                  } else if (this->on_error)
        ...
        try {
            ...
            if (on_error)
                on_error(
                  session->request,
                  make_error_code::make_error_code(errc::operation_canceled));
            return;
        }
    }
};
...
protected:
    void accept() override
    {
        ...
        acceptor->async_accept(
          *connection->socket, [this, connection](const error_code& ec) {
              auto lock = connection->handler_runner->continue_lock();
              if (!lock)
                  return;

              // Immediately start accepting a new connection (unless io_service
              // has been stopped)
