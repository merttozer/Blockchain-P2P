...
public:
    ...
    public:
        static std::string encode(const std::string& ascii) noexcept
        {
            ...
            auto base64_length = static_cast<size_t>(
            ...
            if (BIO_write(b64, &ascii[0], static_cast<int>(ascii.size())) <=
                  0 ||
                BIO_flush(b64) <= 0)
                base64.clear();
                           ...
                           size_t iterations = 1) noexcept
    {
        ...
        hash.resize(128 / 8);
        MD5(reinterpret_cast<const unsigned char*>(&input[0]),
            input.size(),
            reinterpret_cast<unsigned char*>(&hash[0]));

        for (size_t c = 1; c < iterations; ++c)
            MD5(reinterpret_cast<const unsigned char*>(&hash[0]),
                hash.size(),
                reinterpret_cast<unsigned char*>(&hash[0]));
    ...
    static std::string md5(std::istream& stream, size_t iterations = 1) noexcept
    {
        ...
        while ((read_length = stream.read(&buffer[0], buffer_size).gcount()) >
               0)
            MD5_Update(
              &context, buffer.data(), static_cast<size_t>(read_length));
        std::string hash;
        hash.resize(128 / 8);
                            ...
                            size_t iterations = 1) noexcept
    {
        ...
        for (size_t c = 1; c < iterations; ++c)
            SHA1(reinterpret_cast<const unsigned char*>(&hash[0]),
                 hash.size(),
                 reinterpret_cast<unsigned char*>(&hash[0]));
                              ...
                              size_t iterations = 1) noexcept
    {
        ...
        hash.resize(256 / 8);
        SHA256(reinterpret_cast<const unsigned char*>(&input[0]),
               input.size(),
               reinterpret_cast<unsigned char*>(&hash[0]));

        for (size_t c = 1; c < iterations; ++c)
            SHA256(reinterpret_cast<const unsigned char*>(&hash[0]),
                   hash.size(),
                   reinterpret_cast<unsigned char*>(&hash[0]));
                              ...
                              size_t iterations = 1) noexcept
    {
        ...
        hash.resize(512 / 8);
        SHA512(reinterpret_cast<const unsigned char*>(&input[0]),
               input.size(),
               reinterpret_cast<unsigned char*>(&hash[0]));

        for (size_t c = 1; c < iterations; ++c)
            SHA512(reinterpret_cast<const unsigned char*>(&hash[0]),
                   hash.size(),
                   reinterpret_cast<unsigned char*>(&hash[0]));
    ...
    static std::string sha512(std::istream& stream,
                              size_t iterations = 1) noexcept
    {
        SHA512_CTX context;
        SHA512_Init(&context);
