...
public:
    ...
    public:
        ...
        /// When true, this is the last response content part from server for
        /// the current request.
        bool end = true;
...
public:
    class Response
    {
        ...
        Response(std::size_t max_response_streambuf_size,
          ...
          , shared(new Shared())
          , connection_weak(connection_)
          , http_version(shared->http_version)
        ...
        /// Constructs a response object that has empty content, but otherwise
        /// is equal to the response parameter
        Response(const Response& response) noexcept
          : streambuf(response.streambuf.max_size())
          , shared(response.shared)
          , connection_weak(response.connection_weak)
          , http_version(shared->http_version)
          , status_code(shared->status_code)
          , header(shared->header)
          , content(streambuf)
    ...
    public:
        ...
        /// Closes the connection to the server, preventing further response
        /// content parts from server.
        void close() noexcept
        {
...
protected:
    ...
    public:
        Session(std::size_t max_response_streambuf_size,
          ...
          , response(new Response(max_response_streambuf_size, connection))
        {
        }
...
public:
    ...
    /// started in this function. Should not be combined with asynchronous
    /// request functions. If you reuse the io_service for other tasks, use the
    ...
    /// When requesting Server-Sent Events: request_callback might be called
    /// more than twice, first call with empty contents on open, and with ec =
                                    ...
                                    const error_code&)>&& request_callback_)
    {
        ...
        std::weak_ptr<Session> session_weak(
          session); // To avoid keeping session alive longer than needed
        auto request_callback = std::make_shared<
          std::function<void(std::shared_ptr<Response>, const error_code&)>>(
          std::move(request_callback_));
        session->callback =
          [this, session_weak, request_callback](const error_code& ec) {
              if (auto session = session_weak.lock()) {
                  ...
                  }
                  {
                      ...
                      // Remove unused connections, but keep one open for HTTP
                      // persistent connection:
                      std::size_t unused_connections = 0;
                      for (auto it = this->connections.begin();
...
protected:
    ...
    bool synchronous_request_called GUARDED_BY(synchronous_request_mutex) =
      false;

    ClientBase(const std::string& host_port,
               unsigned short default_port) noexcept
    ...
    {
        {
            ...
            if (!synchronous_request_called) {
                if (io_service) // Throw if io_service already set
                    throw make_error_code::make_error_code(
        ...
        request(
          ...
          [&response, &response_promise, stop_future_handlers](
            std::shared_ptr<Response> response_, error_code ec) {
              ...
              if (ec) {
                  response_promise.set_exception(
                    std::make_exception_ptr(system_error(ec)));
    ...
    std::shared_ptr<Connection> get_connection() noexcept
    {
        ...
        if (!host_port) {
            if (config.proxy_server.empty())
                ...
                auto proxy_host_port =
                  parse_host_port(config.proxy_server, 8080);
                host_port =
                  std::unique_ptr<std::pair<std::string, std::string>>(
                    new std::pair<std::string, std::string>(
    ...
    std::pair<std::string, unsigned short> parse_host_port(
      ...
      unsigned short default_port) const noexcept
    {
      ...
      const CaseInsensitiveMultimap& header) const
    {
        ...
        if (corrected_path == "")
            corrected_path = "/";
        if (!config.proxy_server.empty() &&
            std::is_same<socket_type, asio::ip::tcp::socket>::value)
            corrected_path =
              "http://" + host + ':' + std::to_string(port) + corrected_path;
          ...
          *session->connection->socket,
          ...
          [this, session](const error_code& ec, std::size_t bytes_transferred) {
              ...
              if (!ec) {
                  ...
                  auto header_it =
                    session->response->header.find("Content-Length");
                  if (header_it != session->response->header.end()) {
                      auto content_length = std::stoull(header_it->second);
                      if (content_length > num_additional_bytes)
                          this->read_content(
                            session, content_length - num_additional_bytes);
                      else
                          session->callback(ec);
                  } else if ((header_it = session->response->header.find(
                                "Transfer-Encoding")) !=
                      ...
                      this->read_chunked_transfer_encoded(session,
                             ...
                             ((header_it = session->response->header.find(
                                 "Connection")) !=
                                session->response->header.end() &&
                      ...
                      session->callback(ec); // Connection to a Server-Sent
                                             // Events resource is opened
                   ...
                   const error_code& ec)
    {
        ...
        if (it != connections.end()) {
            ...
            session->response = std::shared_ptr<Response>(new Response(
                      ...
                      std::size_t remaining_length)
    {
        asio::async_read(
          *session->connection->socket,
          ...
          [this, session, remaining_length](const error_code& ec,
                                            std::size_t bytes_transferred) {
              auto lock = session->connection->handler_runner->continue_lock();
              if (!lock)
                  return;
              ...
              if (!ec) {
                  if (session->response->streambuf.size() ==
                      ...
                      session->callback(ec);
                      session->response = std::shared_ptr<Response>(
                        new Response(*session->response));
    ...
    void read_content(const std::shared_ptr<Session>& session)
    {
        asio::async_read(
          *session->connection->socket,
          session->response->streambuf,
          [this, session](const error_code& ec_,
                          std::size_t /*bytes_transferred*/) {
              auto lock = session->connection->handler_runner->continue_lock();
              if (!lock)
                  return;
                                ...
                                asio::async_read(
                                  ...
                                  *null_buffer,
                                  ...
                                  [this,
                                   session,
                                   chunk_size_streambuf,
                                   null_buffer](const error_code& ec,
                                                size_t /*bytes_transferred*/) {
                                      auto lock =
      ...
      const std::shared_ptr<asio::streambuf>& events_streambuf)
    {
        asio::async_read_until(
          ...
          *events_streambuf,
          HeaderEndMatch(),
          [this, session, events_streambuf](const error_code& ec,
                                            std::size_t /*bytes_transferred*/) {
              auto lock = session->connection->handler_runner->continue_lock();
              if (!lock)
                  return;
              ...
              if (!ec) {
                  ...
                  session->callback(ec);
                  session->response =
                    std::shared_ptr<Response>(new Response(*session->response));
...
protected:
    ...
    void connect(const std::shared_ptr<Session>& session) override
    {
        if (!session->connection->socket->lowest_layer().is_open()) {
            auto resolver =
              std::make_shared<asio::ip::tcp::resolver>(*io_service);
